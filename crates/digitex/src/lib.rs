...
pub fn into(mut n: u64, d: u32) -> Vec<u32> {
    let mut res = Vec::new();
...
mod test {
    ...
    #[test]
    fn from_raw_test() {
        assert_eq!(Binary::from_raw(18).convert(), vec![1, 0, 0, 1, 0]);
        assert_eq!(Octal::from_raw(18).convert(), vec![2, 2]);
        assert_eq!(Decimal::from_raw(13).convert(), vec![1, 3]);
        assert_eq!(Hexdecimal::from_raw(13).convert(), vec![13]);
    }
    ...
    fn custom_base_test() {
        ...
        let n = Digit::<Tri>::from_raw(18);
        assert_eq!(n.convert(), vec![2, 0, 0]);
    }

    #[test]
    fn stringify_test() {
        let octal = Octal::from_raw(134);
        assert_eq!(octal.to_string(), "206");

        let hex = Hexdecimal::from_raw(28);
